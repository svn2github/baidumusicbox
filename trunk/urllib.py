...
def urlretrieve(url, filename=None, reporthook=None, data=None, seq=None):
    global _urlopener
    if not _urlopener:
        _urlopener = FancyURLopener()
    return _urlopener.retrieve(url, filename, reporthook, data, seq)
def urlcleanup():
    if _urlopener:
        _urlopener.cleanup()
    _safe_quoters.clear()
...
class URLopener:
    ...
    # External interface
    def retrieve(self, url, filename=None, reporthook=None, data=None, seq=None):
        ...
        try:
            ...
            try:
                ...
                if reporthook:
                    reporthook(blocknum, bs, size, seq)
                while 1:
                    block = fp.read(bs)
                    if block == "":
                        break
...
def reporthook(blocknum, blocksize, totalsize, seq):
